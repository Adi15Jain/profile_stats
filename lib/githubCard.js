...
export function renderGitHubSVG({
  ...
  // ---- OVERVIEW PANEL (2x2) ----
  const overviewStats = [
    { label: 'REPOSITORIES',    value: user.public_repos, c: ['#60a5fa', '#22d3ee'], glow: '#3b82f6' },
    { label: 'FOLLOWERS',       value: user.followers,    c: ['#c084fc', '#f0abfc'], glow: '#a855f7' },
    { label: 'TOTAL STARS',     value: totalStars,        c: ['#fbbf24', '#fb923c'], glow: '#f59e0b' },
    { label: 'YEARS ON GITHUB', value: yearsOnGitHub,     c: ['#34d399', '#22d3ee'], glow: '#10b981' },
  ];

  // ---- ACTIVITY PANEL (1x3) ----
  const activityStats = [
    { label: 'CONTRIBUTIONS · 1Y', value: contributions.total,         c: ['#60a5fa', '#818cf8'], glow: '#6366f1' },
    { label: 'CURRENT STREAK',     value: `${contributions.currentStreak}`, suffix: 'd', c: ['#fb923c', '#f43f5e'], glow: '#f97316' },
    { label: 'LONGEST STREAK',     value: `${contributions.longestStreak}`, suffix: 'd', c: ['#34d399', '#22d3ee'], glow: '#10b981' },
  ];

  // Overview cell geometry
  const oCellW = 179, oCellH = 50, oGapX = 14, oGapY = 10;
  const oCols = [24, 24 + oCellW + oGapX];
  const oRowsY = [76, 76 + oCellH + oGapY];

  // Activity panel region
  const aPanelY = 223;
  const aCellW = 121, aCellH = 54, aGapX = 12;
  ...
  const activityCells = activityStats
    ...
    <radialGradient id="aglow-${i}"><stop offset="0%" stop-color="${s.glow}" stop-opacity="0.30"/><stop offset="70%" stop-color="${s.glow}" stop-opacity="0"/></radialGradient>`;
      ...
      const suffix = s.suffix ? `<tspan font-size="12" dx="1">${s.suffix}</tspan>` : '';
